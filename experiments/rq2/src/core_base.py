...
class ReputationScorer:
    ...
    def _dirichlet_uncertainty(self, alpha: np.ndarray) -> float:
        if self._dirichlet is None:
            raise RuntimeError("Dirichlet uncertainty is available only in full mode")
        return 1.0 - self._dirichlet.tv_distance(np.asarray(alpha, dtype=int))
